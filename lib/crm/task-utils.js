...
/**
 * @param {{ isSubTask?: boolean; parentTaskId?: string; parentTaskTitle?: string }} task
 */
export function taskIsSubTaskRow(task) {
  return task.isSubTask === true && Boolean(String(task.parentTaskId ?? "").trim());
}
...
/**
 * KPI-tal for en opgaveliste (typisk efter ansvarlig-filter).
 * @param {Array<{ status: string; dueDate: string; priority: string }>} tasks
 * @param {string} [taskDueReferenceIso]
 */
export function computeTasksSummary(tasks, taskDueReferenceIso = TASK_DEMO_REF_DATE) {
  const openCount = tasks.filter((t) => !taskIsDone(t.status)).length;
  const overdueCount = tasks.filter((t) => taskIsOverdue(t, taskDueReferenceIso)).length;
