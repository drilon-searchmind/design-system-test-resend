...
/** @param {{ assigneeId?: string; assigneeIds?: string[] }} task @param {string} memberId */
export function taskAssignedToMember(task, memberId) {
  const ids =
 ...
 * @param {number} highCount
 * @param {number} overdueCount
 * @param {number} [perfUtil]
 */
export function computeMemberLoadIndex(openCount, highCount, overdueCount, perfUtil = 1) {
  const util = typeof perfUtil === "number" && Number.isFinite(perfUtil) ? perfUtil : 1;
  return Math.min(
    100,
    Math.round(28 + openCount * 11 + highCount * 8 + overdueCount * 6 + (util > 1.02 ? 18 : util > 0.95 ? 8 : 0)),
  );
}
...
export function buildTeamWorkloadRows() {
  return TEAM.map((m) => {
    const open = TASKS.filter((t) => !taskIsDone(t.status) && taskAssignedToMember(t, m.id));
    ...
    const loadIndex = computeMemberLoadIndex(open.length, high, overdue, util);
    return {
