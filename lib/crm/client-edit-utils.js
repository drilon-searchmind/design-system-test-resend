 ...
 */
export function clientToEditDraft(client) {
  ...
  return {
    ...
    retainerAmount: client.retainer != null ? String(client.retainer) : "",
...
export function editDraftToPatch(draft) {
  ...
  /** @type {Record<string, string>} */
  const deptAssignees = {};
  for (const [key, val] of Object.entries(draft.deptAssignees ?? {})) {
    const s = String(val ?? "").trim();
    if (s) deptAssignees[key] = s;
  }
