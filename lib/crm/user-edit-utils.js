...
export function accessTierToPlatformRole(accessTier) {
  return accessTier === ACCESS_TIERS.EXTERNAL_LIMITED ? "readonly" : "member";
}

/** @param {string} tier */
export function accessTierLabel(tier) {
  if (tier === ACCESS_TIERS.EXTERNAL_LIMITED) return agencyPlatformRoleLabel("readonly");
  return agencyPlatformRoleLabel("member");
}
