 ...
 * @param {{ dryRun?: boolean; memberKeys?: string[] }} [opts]
 */
export async function syncMemberDisciplinesFromClients(opts = {}) {
  const dryRun = Boolean(opts.dryRun);
  const memberKeysFilter =
    Array.isArray(opts.memberKeys) && opts.memberKeys.length
      ? new Set(opts.memberKeys.map((key) => String(key ?? "").trim()).filter(Boolean))
      : null;
  await connectDb();
  ...
  for (const member of members) {
    ...
    if (memberKeysFilter && !memberKeysFilter.has(mk)) continue;
