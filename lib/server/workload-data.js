...
import { reportPeriodWindowFromOpts } from "@/lib/server/report-period-window";
...
import { computeMemberLoadIndex } from "@/lib/crm/workload-utils";
...
export async function fetchWorkloadPortfolio(opts = {}) {
  ...
  const { monthStart, monthEnd, monthLastIso, primary: period } = reportPeriodWindowFromOpts(opts);
   ...
   * @param {RawDoc} t
   */
  function assigneeKeysTask(t) {
    const fromArray =
      Array.isArray(t.assigneeMemberKeys) ?
        t.assigneeMemberKeys.map((k) => String(k).trim()).filter(Boolean)
      : [];
    ...
    if (!aid) return [];
    const mk = assigneeOidToKey.get(String(aid)) ?? "";
    return mk ? [mk] : [];
  }
  ...
  for (let mdi = 0; mdi < memberDocs.length; mdi += 1) {
    ...
    const openForMember = openTasksRaw.filter((trow) => assigneeKeysTask(/** @type {RawDoc} */ (trow)).includes(mk));
...
export async function fetchWorkloadMemberDetail(opts = {}) {
  ...
  const assignedMatch = oidOk
    ? /** @type {RawDoc} */ (
        andQuery(scope, {
          $or: [
            { assigneeMemberKey: mk },
            { assigneeMemberKeys: mk },
            { assigneeId: new mongoose.Types.ObjectId(oidStr) },
          ],
        })
      )
    : /** @type {RawDoc} */ (
        andQuery(scope, {
          $or: [{ assigneeMemberKey: mk }, { assigneeMemberKeys: mk }],
        })
      );
  ...
  const clientSlugs = [
    ...new Set(tasksOpen.map((t) => t.clientSlug).filter((s) => typeof s === "string" && s.trim())),
  ];
  /** @type {Map<string, string>} */
  const clientNameBySlug = new Map();
  if (clientSlugs.length) {
    const clientDocs = await Client.find(/** @type {RawDoc} */ (andQuery(scope, { slug: { $in: clientSlugs } })))
      .select("slug name")
      .lean();
    for (let ci = 0; ci < clientDocs.length; ci += 1) {
      const cd = /** @type {RawDoc} */ (clientDocs[ci]);
      const slug = typeof cd.slug === "string" ? cd.slug.trim() : "";
      const name = typeof cd.name === "string" ? cd.name.trim() : "";
      if (slug && name) clientNameBySlug.set(slug, name);
    }
  }

  const tasksOpenEnriched = tasksOpen.map((t) => ({
    ...t,
    clientName: clientNameBySlug.get(t.clientSlug) ?? (t.clientSlug || ""),
  }));
  ...
  let deptPerfUtil = 1;
  if (deptKey) {
    const deptFullRaw = await Department.findOne(/** @type {RawDoc} */ (andQuery(scope, { key: deptKey })))
      .select("monthlyCapacityHours")
      .lean();
    const deptFull = deptFullRaw && typeof deptFullRaw === "object" ? /** @type {RawDoc} */ (deptFullRaw) : null;
    const capacity =
      deptFull && typeof deptFull.monthlyCapacityHours === "number" && Number.isFinite(deptFull.monthlyCapacityHours) ?
        deptFull.monthlyCapacityHours
      : 0;

    const assignedRoll = /** @type {{ hours?: number }[]} */ (
      await Task.aggregate([
        ...
        },
        {
          $group: {
            _id: null,
            hours: { $sum: { $ifNull: ["$estimateHours", 0] } },
          },
        },
      ])
    );
    const assignedHours = typeof assignedRoll[0]?.hours === "number" ? assignedRoll[0].hours : 0;

    const timeRoll = /** @type {{ minutes?: number }[]} */ (
      await TimeEntry.aggregate([
        {
          $match: /** @type {RawDoc} */ (
            andQuery(scope, {
    ...
    const trackedHours = (typeof timeRoll[0]?.minutes === "number" ? timeRoll[0].minutes : 0) / 60;
    const perfUtilRaw =
      capacity > 0 ? trackedHours / capacity : assignedHours > 0 ? trackedHours / Math.max(assignedHours, 1) : 1;
    deptPerfUtil = Number.isFinite(perfUtilRaw) ? perfUtilRaw : 1;
  }

  const loadIndex = computeMemberLoadIndex(openAssigned.length, openHigh, openOverdue, deptPerfUtil);
