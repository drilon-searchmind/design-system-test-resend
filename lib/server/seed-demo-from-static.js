...
import { KNOWLEDGE_ARTICLES } from "@/lib/crm/knowledge-data";
...
export async function seedDemoFromStatic() {
  ...
  for (let ki = 0; ki < KNOWLEDGE_ARTICLES.length; ki += 1) {
    const art = KNOWLEDGE_ARTICLES[ki];
    const author = memberByKey[art.authorId];
    const tags = [...(art.tags ?? [])];
    if (art.categoryId) tags.push(`category:${art.categoryId}`);
  ...
  return {
    ...
    counts: {
      ...
      tasks: TASKS.length,
      knowledgeArticles: KNOWLEDGE_ARTICLES.length,
