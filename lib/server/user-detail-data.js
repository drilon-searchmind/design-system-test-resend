...
import { accessTierToPlatformRole } from "@/lib/crm/user-edit-utils";
...
export async function fetchUserDetailBundle(userAccountId) {
  ...
  const user = {
    ...
    accessTier: tier,
    platformRole: accessTierToPlatformRole(tier),
...
export async function updateUserMongo(userAccountId, body) {
  ...
  const userSet = { email, name, accessTier, provisionedVia, image, clickUpMemberId };
