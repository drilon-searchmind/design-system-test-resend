...
export async function createTaskMongo(body, includeTest, session = null) {
  ...
  try {
    ...
    if (assigneeKeyStr && actor?.userId && taskIdStr) {
...
export async function updateTaskMongo(taskKeyOrId, includeTest, patch, session = null) {
  ...
  const prevAssignee =
    typeof existing.assigneeMemberKey === "string" ? existing.assigneeMemberKey.trim() : "";
  ...
  const newAssignee =
    typeof fresh.assigneeMemberKey === "string" ? fresh.assigneeMemberKey.trim() : "";
  const taskTitle = typeof fresh.title === "string" ? fresh.title : "Opgave";

  if (session && newAssignee && newAssignee !== prevAssignee && taskIdStr) {
    const actor = await resolveTaskActorFromSession(session);
    if (actor.userId) {
      await notifyTaskAssigned({
        taskId: taskIdStr,
        taskTitle,
        assigneeMemberKey: newAssignee,
        actor,
        scope: /** @type {Record<string, unknown>} */ (scope),
        isTest: includeTest,
      });
