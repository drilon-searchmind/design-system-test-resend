...
import {
  ...
  buildTasksPortfolioMongoFilterForPeriods,
...
/**
 * @param {{ includeTest?: boolean; year?: number; month?: number; periods?: Array<{ year: number; month: number }>; mineAssigneeKey?: string }} opts
 */
export async function fetchTasksPortfolio(opts = {}) {
  ...
  const periods =
    Array.isArray(opts.periods) && opts.periods.length ?
      opts.periods.map((p) => normalizeReportPeriod(p))
    : [{ year, month }];
  ...
  const periodOr = buildTasksPortfolioMongoFilterForPeriods(periods);
